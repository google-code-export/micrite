		...
		for (Role role : roles) {
			...
			for (Authority auth : auths) {
				...
				if (StringUtils.indexOf(auth.getName(), "/")>=0){
					String tmp = "";
					/* 根据选定节点，匹配拥有的权限串，生成下级节点的id和text，
					 * 例：当前User拥有3个权限串( "/N1/N2/N3" , "/N1/N4" 和 "/N5" )
					 * 选定根节点：全部3个权限串的处理都直接split，取第一个token，最后得到"N1"和"N5"
					 *           会得到2次"N1"，没关系，因为使用Set类型，所以重复的对象不会被add，
					 *           注意map要put相同的值(比如url)，如果有不同的值被认为不同的对象
					 * 选定中间节点N1: 第1,2个权限串先substring，得到"/N2/N3"和"N4"，然后再split，最后得到"N2"和"N4"
					...
					if(names.length>1){
						...
						map.put("url", StringUtils.substring(auth.getValue(),1));  
						map.put("leaf", true);						
					}
					menu.add(map); 		
				}
